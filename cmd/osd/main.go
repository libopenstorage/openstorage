...
func start(c *cli.Context) error {
	...
	if ll, err := logrus.ParseLevel(c.String("loglevel")); err != nil {
		logrus.Warnf("logging level passed in is incorrect: %q", c.String("loglevel"))
	} else {
		logrus.SetLevel(ll)
	}
	if c.String("logformatter") == "TXT" {
