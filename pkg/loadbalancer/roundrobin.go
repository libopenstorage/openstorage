...
// RoundRobin provides APIs to load balance a gRPC connection over a given
// cluster.
type RoundRobin interface {
	// GetGrpcConnection returns a gRPC client connection to a node
	// in the cluster using a round-robin algorithm. It will return
	// an error if it fails to create a connection to a node in the cluster.
	GetGrpcConnection(ctx context.Context) (*grpc.ClientConn, error)

	// Stop stops the round-robin balancer
	Stop()
}
...
func (rr *roundRobin) GetGrpcConnection(ctx context.Context) (*grpc.ClientConn, error) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	...
	rr.nextCreateNodeNumber = (targetNodeNumber + 1) % len(nodesResp.Nodes)
	...
	return rr.connMap[targetNodeEndpoint].Conn, nil
