...
func TestFakeCreateVolume(t *testing.T) {
	...
	vols, err := d.Inspect(context.TODO(), []string{vid}, nil)
	assert.NoError(t, err)
...
func TestFakeInspect(t *testing.T) {
	...
	v, err := d.Inspect(context.TODO(), []string{"asdf"}, nil)
	assert.NotNil(t, err)
	assert.Error(t, err)
...
func testInitForCloudBackups(t *testing.T, d *driver) (string, string, *api.CloudBackupCreateRequest, *api.Volume) {
	...
	origvols, err := d.Inspect(context.TODO(), []string{volid}, nil)
	assert.NoError(t, err)
...
func TestFakeCloudBackupRestore(t *testing.T) {
	...
	vols, err := d.Inspect(context.TODO(), []string{resp.RestoreVolumeID}, nil)
	assert.NoError(t, err)
	assert.Len(t, vols, 1)
	vol := vols[0]
...
func TestFakeSet(t *testing.T) {
	...
	// Verify
	vols, err := d.Inspect(context.TODO(), []string{volid}, nil)
	assert.NoError(t, err)
	assert.Len(t, vols, 1)
