...
func TestFakeCreateVolume(t *testing.T) {
	...
	vols, err := d.Inspect([]string{vid}, false)
	assert.NoError(t, err)
...
func TestFakeInspect(t *testing.T) {
	...
	v, err := d.Inspect([]string{"asdf"}, false)
	assert.NotNil(t, err)
	assert.Error(t, err)
...
func testInitForCloudBackups(t *testing.T, d *driver) (string, string, *api.CloudBackupCreateRequest, *api.Volume) {
	...
	origvols, err := d.Inspect([]string{volid}, false)
	assert.NoError(t, err)
...
func TestFakeCloudBackupRestore(t *testing.T) {
	...
	vols, err := d.Inspect([]string{resp.RestoreVolumeID}, false)
	assert.NoError(t, err)
	assert.Len(t, vols, 1)
	vol := vols[0]
