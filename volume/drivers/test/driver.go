...
func inspect(t *testing.T, ctx *Context) {
	...
	vols, err := ctx.Inspect([]string{ctx.volID}, false)
	...
	vols, err = ctx.Inspect([]string{string("shouldNotExist")}, false)
...
func snapInspect(t *testing.T, ctx *Context) {
	...
	snaps, err := ctx.Inspect([]string{ctx.snapID}, false)
	require.NoError(t, err, "Failed in Inspect")
	...
	snaps, err = ctx.Inspect([]string{string("shouldNotExist")}, false)
