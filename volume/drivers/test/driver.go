...
func inspect(t *testing.T, ctx *Context) {
	...
	vols, err := ctx.Inspect(correlation.TODO(), []string{ctx.volID}, nil)
	...
	vols, err = ctx.Inspect(correlation.TODO(), []string{string("shouldNotExist")}, nil)
...
func snapInspect(t *testing.T, ctx *Context) {
	...
	snaps, err := ctx.Inspect(correlation.TODO(), []string{ctx.snapID}, nil)
	require.NoError(t, err, "Failed in Inspect")
	...
	snaps, err = ctx.Inspect(correlation.TODO(), []string{string("shouldNotExist")}, nil)
