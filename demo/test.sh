...
kubectl apply -f e2e/storageclass.yaml || fail "failed to apply storageclass yaml"
kubectl get storageclass openstorage-sc || fail "failed to create storageclass"

# Create PVC
kubectl apply -f e2e/pvc.yaml || fail "failed to apply PVC yaml"
kubectl get pvc openstorage-pvc || fail "failed to create pvc"

# Use PVC with MySQL deployment
kubectl apply -f e2e/mysql.yaml || fail "failed to apply deployment yaml"
