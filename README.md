...
2. Add your driver `mydriver` implementation in the `drivers/mydriver` directory.  The driver must implement the `VolumeDriver` interface specified in `volumes/volumes.go`.  This interface is an implementation of the specification available at https://github.com/libopenstorage/specs/blob/master/volume/
