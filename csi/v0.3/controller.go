...
func (s *OsdCsiServer) ValidateVolumeCapabilities(
	...
	volumes, err := s.driver.Inspect([]string{id}, false)
...
func (s *OsdCsiServer) DeleteVolume(
	...
	volumes, err := s.driver.Inspect([]string{req.GetVolumeId()}, false)
	if (err == nil && len(volumes) == 0) ||
...
func (s *OsdCsiServer) DeleteSnapshot(
	...
	volumes, err := s.driver.Inspect([]string{req.GetSnapshotId()}, false)
	if (err == nil && len(volumes) == 0) ||
		(err != nil && err == kvdb.ErrNotFound) {
